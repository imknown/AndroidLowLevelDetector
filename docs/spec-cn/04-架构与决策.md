...
- 条目级检测目录的正式定义 ("[检测条目目录](05-检测条目目录.md)", 已定稿 2026-09-22; 本篇不重复).
...
- **检测器契约**: 目录里的每个条目 ("[检测条目目录](05-检测条目目录.md)") 实现一个小契约 — 自己是谁 (id, 类别, 标题), 回答什么问题, 以及返回**三态结果 + 证据**(读到的原始值, 用的什么手段) 的 `execute()`.
