...
| [issues-cn/](issues-cn/README.md) | Project issues summary: the View-era architecture review's findings (`AR-xx`, `C`/`R`/`A`), the quick-win fixes (`QW-x`), and the Compose-migration code reviews (`F-x`) all feed the backlog | **record + backlog** — re-verify a finding against current code before acting on it; the View-era descriptions are history, don't back-fill today's code into them |
