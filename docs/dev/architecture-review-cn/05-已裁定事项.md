...
**2026-09-22 复验（追加，不改上文）**:上面的「实现只切 `ViewExt.kt:8-15`」已经不存在——`ViewExt.kt` 随 View 层一起删除，列表侧**没有任何订阅者**：`SettingsViewModel.scrollBarModeChangedSharedFlow 与 emitScrollBarModeChangedSharedFlow()` 照旧 emit `scrollBarModeChangedSharedFlow`，全仓零 collector（`SettingsScreen() 的 onScrollBarSelect` 只把值写进 SharedPreferences）。所以缺口比原记的更大：不是「只有第三档是 stub」，而是**三档全不生效**。`arrays.xml` 的结构与默认值仍与上文一致。
