...
若页面重建恰好落在下拉拖拽进行中（View 时代是 Activity/Fragment 重建，Compose 侧对应 `NavDisplay` 的 entry 重建与 Activity 配置变更）,`onRefresh` 永不触发、没有任何请求存在,重建后的视图展示旧列表。手势重放的修复曾被构建、真机验证,又因过于侵入而回退。影响 FR-5 AC1/AC2 的用户感知。

**处理(决策⑤,已批准)**:永久接受。「显示过期」的一半已在 2026-09-21 的排序重构里治好——`HomeViewModel.kt:71-102` 实时同步 + `:245-251` 的 `onModelsLoaded` 兜底;不再排期「待处理刷新重放」特性。注:当时复验的是 View 版手势,Compose 侧的同一场景没有重新复验过,复现时按本裁定同样接受。
...
规格要求「顶栏逐渐推出屏幕**且**底栏隐藏」。Compose 侧 `AppRoot.kt:141-166` 的 `Scaffold` 里 `TopAppBar` 没有 `scrollBehavior`，`NavigationBar` 同样固定——**两个都不隐藏**，规格 AC1 的「可中断动画」因此完全无从谈起。View 时代那份证据（`main_activity.xml` 的 AppBarLayout 缺 `layout_scrollFlags`、只有 BNV 挂 `hide_bottom_view_on_scroll_behavior`，所以至少底栏会隐藏）已随迁移删除，当时「只对上了一半」的那一半现在也没了。

**处理(决策④,已批准)**:修订为 Compose 时代交付物,不在 XML 里做(会被 Compose 重写丢弃)。文档已修订,Compose 迁移时兑现。**2026-09-22 复验**:迁移已完成,本项仍未兑现,且缺口比 View 时代更大(见上)。
...
## R10 滚动条三档设置在 Compose 迁移后全部不生效

**严重程度:P2(🟢) ｜ 处理:决策③ + 2026-09-22 负责人复述**

需求是三档:**无 / 通常 / 可拖拽**。View 时代只实现了前两档(`ViewExt.setScrollBarMode` 切 `RecyclerView.isVerticalScrollBarEnabled`),「可拖拽」从未实装,只在 `arrays.xml` 留了 string 值——`scrollBarKeys` 的第三项是注释状态(labels 2 项 / values 3 项),设置页选不到它,默认值为 None。这一半就是决策③「先留着不删」裁定所指的东西。

Compose 迁移把 `ViewExt.kt` 连同 View 层一起删了,列表页侧**没有任何订阅者**:`SettingsViewModel.kt:36-44` 仍照旧 emit `scrollBarModeChangedSharedFlow`,全仓零 collector(`SettingsScreen.kt:117-121` 只把值写进 SharedPreferences)。所以现在不是「第三档是 stub」,而是**三档全都不生效**。

**处理(2026-09-22 负责人定)**:保持代码现状,等官方方案——material3 的滚动条组件目前只在 alpha 线(不在 Compose BOM 内),既不引入 alpha 覆盖 BOM,也不自绘。设置项、`arrays.xml` 三档值与那条 emit 一并留着,官方组件转正后把值接上即可;`SettingsScreen.kt:120` 的注释已按现状改写(不再声称"列表页立即生效")。`fast` 档继续保持注释状态。
