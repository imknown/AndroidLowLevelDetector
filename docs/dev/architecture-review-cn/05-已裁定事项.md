...
**2026-09-22 复验（追加，不改上文）**:载体已换成 Compose——没有 Fragment,下拉是 `MyModelListScreen.kt:73-76` 的 `PullToRefreshBox`,进页面由 `LaunchedEffect { viewModel.init() }` 触发。上文那句「治好的一半」已经提前落地:排序一项改由 `HomeViewModel.kt:71-102` 实时同步 + `:245-251` 的 `onModelsLoaded` 兜底。但 Compose 侧「拖拽期间重建」这个场景本身没有重新复验过。裁定不变:永久接受,不排期刷新重放。
...
**2026-09-22 复验（追加，不改上文）**:证据载体 `main_activity.xml` 已随迁移删除，而当时的行为没有延续——Compose 侧 `AppRoot.kt:141-166` 的 `Scaffold` 里 `TopAppBar` 没有 `scrollBehavior`，`NavigationBar` 同样固定，**两个都不隐藏**：AC1 的「可中断动画」从「只对上一半」退成「完全没有」。裁定不变（仍算 Compose 交付物），但工作量比当年估的大：底栏那一半也得重做。
...
**2026-09-22 复验（追加，不改上文）**:上面的「实现只切 `ViewExt.kt:8-15`」已经不存在——`ViewExt.kt` 随 View 层一起删除，列表侧**没有任何订阅者**：`SettingsViewModel.kt:36-44` 照旧 emit `scrollBarModeChangedSharedFlow`，全仓零 collector（`SettingsScreen.kt:117-121` 只把值写进 SharedPreferences）。所以缺口比原记的更大：不是「只有第三档是 stub」，而是**三档全不生效**。`arrays.xml` 的结构与默认值仍与上文一致。

**处理(2026-09-22 负责人定)**:保持代码现状,等官方方案——material3 的滚动条组件目前只在 alpha 线(不在 Compose BOM 内),既不引入 alpha 覆盖 BOM,也不自绘。设置项、`arrays.xml` 三档值与那条 emit 一并留着,官方组件转正后把值接上即可(届时是从零接入，不是「一行替换」);`SettingsScreen.kt:120` 的注释已按现状改写(不再声称"列表页立即生效")。`fast` 档继续保持注释状态。
