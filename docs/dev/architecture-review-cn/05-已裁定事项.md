...
**2026-09-22 复验（追加，不改上文）**:证据载体 `main_activity.xml` 已随迁移删除，而当时的行为没有延续——Compose 侧 `AppRoot.kt:141-167` 的 `Scaffold` 里 `TopAppBar` 没有 `scrollBehavior`，`NavigationBar` 同样固定，**两个都不隐藏**：AC1 的「可中断动画」从「只对上一半」退成「完全没有」。裁定不变（仍算 Compose 交付物），但工作量比当年估的大：底栏那一半也得重做。
