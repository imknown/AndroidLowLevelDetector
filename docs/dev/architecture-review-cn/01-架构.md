...
`HomeRepository.detectPerformanceClass()`（`app/src/main/java/net/imknown/android/forefrontinfo/ui/home/repository/HomeRepository.kt:292`）——一个「数据层」方法，干的却是界面的活：
...
```kotlin
// app/src/main/java/.../ui/base/list/MyModel.kt:17
@Immutable
data class MyModel(
...
`MyApplication` 伴生对象身兼七职（`MyApplication.kt:40`）：全局实例、SharedPreferences 持有者、字符串工具、下载目录工具、主题逻辑、Shell/Property 装配……全项目靠它伸手取物：
...
接线写在四个 ViewModel 各自的伴生 `Factory` 里（`HomeViewModel.kt:42-48`）：
...
`OthersViewModel.kt:25-36` 一口气 new 7 个对象（6 个 DataSource + 1 个 Repository），`PropViewModel.kt:19-25`、`SettingsViewModel.kt:29-34` 各自再来一套。
...
View 时代的 `MY_REPOSITORY_KEY` + `extrasProducer` 样板已随 Navigation 3 迁移删除（入口改为 `viewModel(factory = ...)`，`AppRoot.kt:102-105`），所以当年「四个 Fragment 各写一份接线」剩下的问题只有一个更小的版本：**同一份接线在四个 ViewModel 伴生对象里各写一遍**。注意：这一步必须在 AR-04 完成容器之后再做，否则只是把 `new` 换了个地方。
...
- **未知/不可读值通常塌缩为红色**——例如 Android 版本未知时 `lld == null -> StatusColor.CRITICAL`(`HomeRepository.kt:143-148`,复验);仅 GSI「未识别」映射为黄色。
...
- 唯一的行模型是 `MyModel(title, detail, color: StatusColor, type)`——「级别」就是展示用状态色;**不存在 支持/不支持/未知 类型**(「支持/不支持」只是本地化的详情字符串,`HomeRepository.kt:1142-1148`)。FR-3 的模型与 ADR-005 的「Unknown 是一等结果」在代码中没有表示。
- 目录是字面的 `tempModels += repository.detectXxx()` 序列(`HomeViewModel.kt:199-229`;`OthersViewModel.kt:42-112`;`PropViewModel.kt:27-41`)——增删条目 = 改函数体,正是 FR-1 AC2 / ADR-008 禁止的。
