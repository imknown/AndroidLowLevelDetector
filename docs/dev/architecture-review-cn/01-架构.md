...
颜色这一半在 Compose 迁移里已经收敛 (`R.attr` 主题句柄 → `StatusColor` 枚举, 见 [AR-11](04-反模式与隐患.md#AR-11)), 但**"由数据层决定用哪个颜色" 这件事本身没变** — 变的只是句柄的类型.
...
- **反向依赖**: 数据层反过来依赖 UI 资源 (`R`), 违反了 "依赖只能从外层指向内层" 的分层规则, 也是 [AR-07](01-架构.md#AR-07) 包循环的一半成因.
...
- 删除 `ShellManager`, `PropertyManager` 这两层薄委托壳. `ShellDefault` **不在此列** — 它是不依赖 libsu 的原生备选实现, [AR-13.6](04-反模式与隐患.md#AR-13) 裁定保留.
- `getMyString` 的 91 处调用随 [AR-01](#AR-01) 的 mapper 迁移自然消亡: mapper 拿 `Context` 参数, 不再需要全局函数.
...
在 [AR-04](01-架构.md#AR-04) 的 `AppContainer` 之上, 四个 Factory 退化成 "取现成的":
...
只搬包不改逻辑. 目标结构 (配合 [AR-03](02-SSOT-唯一数据来源.md#AR-03),[AR-04](01-架构.md#AR-04) 一并落位):
...
迁移清单 (一次提交): 移动 7 个文件 + 全局改 import.(原先顺手列的 "删除 `ShellDefault`" 已按 [AR-13.6](04-反模式与隐患.md#AR-13) 的裁定取消 — 它作为原生备选实现保留.) IDE 的 Refactor → Move 可全自动完成.
...
与 [AR-07](01-架构.md#AR-07) 同根: 缺一个中立的共享层 (core), 跨功能知识只能寄生在某个功能包里, 名字再错也只能将就.
...
两个 `FingerprintDataSource` 分别更名为 `PartitionFingerprintDataSource` 与 `SigningCertDataSource` (或随 [AR-07](01-架构.md#AR-07) 的包重排迁入各自功能后自然消歧). 随 AR-07 的机械搬移一并完成, 不单独开提交.
...
**裁定 2026-09-06(Q6=A)**: 有界并发维持为目标; §3.3 同时钉死**固定展示顺序**与检测器依赖. 落在注册表引擎落地时 (与 [AR-12](04-反模式与隐患.md#AR-12) 的调度器问题同属检测执行模型, 但相互独立).
