...
- 目录计数与渲染行数不符: Home = 23 个检测器调用但 **25 行**(`detectSecurityPatches` 产 2 行, `detectTrebleAndGsiCompatibility` 产 2 行, 复验);Others = 31 固定 + 6 个 API 条件项 + N 个分区指纹动态行; Prop = 5 个检测器各产动态行. 目录 (见 [docs/spec-cn/05](../../spec-cn/05-detection-item-catalog.md)) 预填的值是 23/37/5.
