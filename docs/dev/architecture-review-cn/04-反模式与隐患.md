...
状态只有两条流，没有失败通道（`BaseListViewModel.kt:21-27`）：
...
加载不设防（`BaseListViewModel.kt:55-66`）——`collectModels()` 一旦抛出未捕获异常，后果比"协程死亡"更严重（见下方直接原因）：
...
而界面把「正在转圈」绑在那条布尔流上（`MyModelListScreen.kt:41,54`）：
...
字符串资源 ID 被当成枚举做逻辑判别（`HomeRepository.detectMode`，`HomeRepository.kt:63`）：
...
- `Version.id` 让界面层不得不反向持有格式化知识（`SettingsScreen.kt:236` 按 `v.id` 取字符串），数据类里混入了界面指令。
...
改动点两处引用：`SettingsScreen.kt:236`（`v.id`）与预览构造 `SettingsScreen.kt:415`（`id = R.string.about_version_summary`）。

`MyModel` 里的 `MyModelTitle.Res` 属于「界面模型携带资源 ID」，与 AR-01 的 mapper 方案一致（mapper 就在 UI 层，拿资源 ID 是合法的），不需要改。同一条里的 `color: @AttrRes` 那一半已经在 Compose 迁移里落地成 `StatusColor` 枚举（`MyModel.kt:17-23`），不再需要改。
...
阻塞式 Shell / 属性调用被包在 `Dispatchers.Default`（CPU 专用池）里执行（`HomeViewModel.kt:199 / 203 / 258`）：
...
四处用 `>=` / `<` 直接比较字符串，正确性完全依赖格式恒定（`HomeRepository.kt:270` 安全补丁、`:624-625` Mainline、`:662` VNDK、`LldManager.kt:47` lld 版本）。零填充的 ISO 日期目前碰巧正确，但位数一变（如 `9` 对 `10`）即静默出错：
...
`MyModelCard` 用 `stringResource(title.id)` 渲染标题（`MyModelCard.kt:63`）——标题每次组合按当前语言现解析，`detail` 却是检测时冻结好的字符串（`MyModelCard.kt:92-95` 只排版不解析）：系统切换语言后，同一行会显示「新语言标题 + 旧语言详情」，直到手动刷新。View 时代这件事发生在 `MyAdapter.onBindViewHolder`（每次绑定调 `MyApplication.getMyString(title.id)`），载体换成了 Composable，缺陷原样搬了过来。[AR-01](01-架构.md#AR-01) 的 mapper 落地之后，标题与详情统一在数据到位时解析一次，卡片只读现成字符串。

### 10. Easter egg 计数器放在 ViewModel

`SettingsViewModel.timesLeft`（`SettingsViewModel.kt:70`）是可变字段 + 返回资源 ID 的查询函数——「连点 7 次版本号」的交互细节属于界面层。挪进 `SettingsScreen`（一个 `remember` 计数器即可），或改为不可变状态流转。
...
先用反射取出 `Settings.System/Secure/Global` 类里的**常量字段名**当作 key 列表（`SettingsDataSource.kt:10`）：
...
而 `getStringOrNullOrThrow` 本身又是一层反射（`SettingsDataSource.kt:21-22`）：`getDeclaredMethod("getString", …).invoke(null, …)`。
...
```kotlin
// base/extension/DateTimeExt.kt:9-10 —— 契约 A：解析 pattern
...
而联网路径上，`tryDetectOnline` 对取数、反序列化、保存都有 try/catch，**唯独最后的 `detect()` 调用没有**（`HomeViewModel.kt:146`）；`detectMode` 第一行就调用这个格式化（`HomeRepository.kt:63`）：
...
而判定层靠**比较这些文案的内容**来识别状态，全项目两处（`HomeRepository.kt:1136-1140`、`OthersRepository.kt:150`）：
...
- `HomeViewModel.detect()`(`app/src/main/java/.../ui/home/HomeViewModel.kt:194-232`)顺序调用 23 个 `detect*` 探针,**无任何逐探针 try/catch**;只有网络/解析/保存编排层有 catch。
...
- **在线路径不做版本比较**:成功的网络拉取无条件覆盖缓存(`HomeViewModel.kt:135-146`)。这是当前规格允许的行为(服务端副本通常较新),但意味着过期的服务端文件会降级本地较新的缓存——该风险的缓解(发布时更新分支 json 或钉到 tag)已记录在规格的风险表中,不需要代码改动。
...
- 硬编码预览偏移 `250205 - 250101`(`HomeRepository.kt:225-232`);
