...
`MyApplication.setMyTheme` 使用的 `AppCompatDelegate.MODE_NIGHT_AUTO_BATTERY` 在新版 AppCompat 中已标记弃用（Android 10 起省电暗色由系统接管，`MODE_NIGHT_FOLLOW_SYSTEM` 已含该行为）。随 [AR-02](02-SSOT-唯一数据来源.md#AR-02) 的设置收拢一并处理：删掉该分支或映射到 `FOLLOW_SYSTEM`。
