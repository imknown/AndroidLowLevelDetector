...
一次性事件走 `Channel` + `receiveAsFlow()`, 不要再开一条 `StateFlow`: 错误是要被消费掉的通知, 不是可重放的状态; 而 `SharedFlow` (replay=0) 在没人订阅时直接把事件丢掉 — 本报告的 [F1](A-Compose代码Review报告.md#F1) 就是现成的教训. 流挂在 ViewModel 实例上, 绝不放伴生对象 (那是 [AR-02](02-SSOT-唯一数据来源.md#AR-02) 的坑).
