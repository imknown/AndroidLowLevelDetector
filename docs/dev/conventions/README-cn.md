...
- 命令执行用 libsu 的 **非 root** 模式 (`ui/common/ShellLibSu.kt`, 带 `Shell.FLAG_NON_ROOT_SHELL`): 还没有 root 层, 见架构体检 [A4](../architecture-review-cn/05-已裁定事项.md#A4).
...
- `ShellDefault` 故意没有调用方: 它是保留备用的原生 shell 实现, 不依赖 libsu (在用的是 `ShellLibSu`). 别把它当死代码删; 如果将来启用它, 先修掉 `waitFor()` 之后读管道的死锁 (架构体检 [AR-18.1](../architecture-review-cn/04-反模式与隐患.md#AR-18)).
