...
```kotlin
// LldDataSource.kt → LldDataSource.fetchOnlineLldJsonStringOrThrow() —— 每次调用新建,用完即弃
val client = HttpClient(OkHttp) { engine { config { ... } } ... }

// LldDataSource.kt → LldDataSource.fetchOnlineLldJsonStringOrThrow() —— 拿到响应直接读 body,不查状态码
