...
- 合入前先在本地各跑一遍:若存量测试或 lint 有 error,CI 首跑会变红,需要先清掉或单独裁定。另注:AGP 自带 lint 默认**不会**把 Kotlin 弃用告警(如 QW-4 的常量)判成 error——想让这类问题在 CI 拦截,需另行配置(如 `warningsAsErrors`),不在本条范围。
...
联网路径上取数、解析、保存都有 try/catch,唯独最后的 `detect()` 裸奔(`HomeViewModel.kt:92`):
...
## QW-4 暗色主题用了已弃用的常量(AR-18.4 MODE_NIGHT_AUTO_BATTERY 已弃用)

**改动量:2 行 ｜ 文件:`app/.../base/MyApplication.kt`**

### 问题代码

```kotlin
// MyApplication.kt:53-55
getMyString(R.string.interface_themes_power_saver_value) -> {
    AppCompatDelegate.MODE_NIGHT_AUTO_BATTERY      // ← 新版 AppCompat 已弃用
}
```
...
| 日期 | 提交信息 | 当时目的 |
|---|---|---|
| 2021-01-23 | Fix "lateinit property instance has not been initialized" on some high-end devices | 主题分支随那次 Application 重构落成(推断:当时该常量尚未弃用) |

### 修改后(快赢版:保留行为,只压告警)

```kotlin
// minSdk 23:该常量在 API 28- 上仍有效(省电暗色由 AppCompat 自己查省电模式实现),
// 保留行为、只压弃用告警;是否删该选项随 AR-02(设置项绕开唯一数据源)设置收拢时裁定
...
注解必须放在 `setMyTheme` 函数声明上——放在 when 分支条件前不是合法的 Kotlin 语法,放在伴生对象属性上也覆盖不到分支内的引用。

为什么**不**按评审原方案映射到 `MODE_NIGHT_FOLLOW_SYSTEM`:minSdk 是 23,而 `AUTO_BATTERY` 的"省电时暗色"是 AppCompat 自己查省电模式实现的,在 API 28- 上同样生效;`FOLLOW_SYSTEM` 在 Android 10 之前等同于"永远浅色"。直接替换会让老设备用户**失去省电暗色功能**,且设置项文案与四份翻译仍写着"省电时暗色",语义也对不上——这是行为变化,需要负责人连同「是否干脆删掉该选项」一起裁定,随 AR-02(设置项绕开唯一数据源,靠静态事件总线广播)设置收拢时处理更合适。快赢版零行为变化。

### 直接原因

AppCompat 把 `MODE_NIGHT_AUTO_BATTERY` 标记弃用:省电暗色策略归系统管,应用侧不应再自己判。

### 根本原因

「枚举选项 → 系统常量」的映射表写死后没人回头审;弃用告警默认不会让 CI 变红(见 QW-1 的边界说明),坏了也没人看见。
