...
```kotlin
// HomeViewModel.kt → HomeViewModel.detect()、203 —— 20+ 个探针内部是 Shell.cmd().exec()、/proc 读取、反射
...
```kotlin
// HomeRepository.kt → HomeRepository.detectMode()
...
```kotlin
// SettingsRepository.kt → SettingsRepository.Version(构造处 :88 传入了 R.string.about_version_summary)
data class Version(
