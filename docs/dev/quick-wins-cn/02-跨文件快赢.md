...
`Version` 删掉 `id` 字段、构造处(`SettingsRepository.kt:87-95`)删掉 `R.string.about_version_summary` 实参;界面自己知道自己该用哪条文案:

```kotlin
// SettingsScreen.kt:236 —— 版本行的 supporting 文案
Text(
    stringResource(
...
还有一处构造点要跟着改:预览里的 `SettingsScreen.kt:415`(`id = R.string.about_version_summary`)。
