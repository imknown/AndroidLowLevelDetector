...
```kotlin
// HomeViewModel.kt:145、149 —— 20+ 个探针内部是 Shell.cmd().exec()、/proc 读取、反射
withContext(Dispatchers.Default) {
    tempModels += homeRepository.detectAndroid(lld)
    ...
}

// HomeViewModel.kt:192、SettingsRepository.kt:34、47、69 —— packageManager 查询是跨进程 binder 调用
// OthersViewModel.kt:39、PropViewModel.kt:36 —— collectModels 内含 shell 调用
```
...
8 处 `Dispatchers.Default` 全部改为 `Dispatchers.IO`,其余一字不动。`HomeViewModel.kt:157` 内层已有 `withContext(Dispatchers.IO)`,保留(无害)。
