...
abstract class BaseListFragment : BaseFragment<BaseListFragmentBinding>() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            listViewModel.modelsStateFlow.flowWithLifecycle(viewLifecycleOwner.lifecycle).collect { myModels ->
                myModels ?: return@collect

                listViewModel.showModels(myAdapter.myModels, myModels)
            }
        }

        viewLifecycleOwner.lifecycleScope.launch {
            listViewModel.showModelsEventStateFlow.flowWithLifecycle(viewLifecycleOwner.lifecycle).collect {
                it ?: return@collect
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            listViewModel.showErrorEventStateFlow.flowWithLifecycle(viewLifecycleOwner.lifecycle).collect { message ->
