...
fun SettingsScreen(
    ...
    val scrollBarModeSetting = remember(sharedPreferences, SettingsViewModel.scrollBarModeChanged) {
        sharedPreferences?.getString(scrollBarKey, scrollBarDefaultValue) ?: scrollBarDefaultValue
    }
...
fun ListPreferenceComposable(
    ...
    LaunchedEffect(sharedPreferences, key) {
        sharedPreferences?.let {
            currentValue = it.getString(key, defaultValue) ?: defaultValue
        }
    }
...
fun SwitchPreferenceComposable(
    ...
    LaunchedEffect(sharedPreferences, key) {
        sharedPreferences?.let {
            isChecked = it.getBoolean(key, defaultValue)
        }
    }
