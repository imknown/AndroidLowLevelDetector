...
Guidance for coding agents working in this repository. Read this before making changes; if this file ever disagrees with the code, trust the code and update this file. Conventions that deliberately diverge from the mainstream / official template are called out inline — they are not smells to fix, so don't "normalize" them in passing.

## Project overview

Android app that surfaces low-level system characteristics of the device: Treble and GSI compatibility, Mainline/APEX modules, system-as-root, A/B partitions, Binder bitness, security patch levels, and related build properties.

Stack: Kotlin, Jetpack Compose (single Activity, Navigation 3), MVVM with unidirectional data flow (coroutines + StateFlow), kotlinx.serialization, Ktor for networking, libsu for shell access, JNI/NDK for binder protocol detection. No DI framework.

- Product introduction and downloads: [README.md](README.md)
- Application id `net.imknown.android.forefrontinfo`; version info lives in `gradle/toml/build.toml` (`versionCode` / `versionName`).
...
- `build-logic` — included build holding the convention plugins; the single source of SDK and build values for all modules. Shared configuration (SDK, desugaring, Java toolchain, Kotlin compiler args, test dependencies) lives in `build-logic/convention/src/main/kotlin/.../android/`; modules only apply plugins.
...
- Root command execution uses libsu (`ui/common/ShellLibSu.kt`).
...
- The global `myAndroid` (`AndroidVersionExt`) is written only at its designated initialization in `HomeRepository`; never assign to it elsewhere — the `isAtLeast…()` helpers read it from everywhere.
...
- PRs target `develop` (the default branch). Commit messages follow Conventional Commits with **lowercase type + scope**: `fix(home): ...`, `refactor(settings): ...`, `chore(deps): ...`.
...
- The current `jetpack-compose-new` branch is the View→Compose migration described in `docs/dev/compose-migration-plan-cn/`.

## Documentation

- Developer docs live under `docs/dev/`. Documents in any language are welcome, but the directory name must carry a language suffix — Chinese uses `-cn` (for example `compose-migration-plan-cn/`).
- Read the migration plan's "key decisions" summary before touching UI, to avoid contradicting settled decisions. `JOTTINGS.md` holds ideas and todos.
...
- Docs, comments, and sample code must not contain real signatures, keys, passwords, or other sensitive values.
