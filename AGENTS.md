...
- **English by default**: anything you generate — docs, comments, commit messages — is in English unless the user specifies otherwise. Whatever language a doc is written in, its punctuation is ASCII, spaced as in English (no `，。、：（）「」`): see [docs/README.md](docs/README.md#conventions).
